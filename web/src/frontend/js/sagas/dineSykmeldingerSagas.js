...
function* hentDineSykmeldinger() {
    log('Utfører restkall - dine sykmeldinger');
...
export function* hentSykmeldingerHvisIkkeHentet() {
    const skalHente = yield select(skalHenteDineSykmeldinger);
    if (skalHente) {
        log('Henter sykmeldinger');
        yield hentDineSykmeldinger();
    }
}

function* watchHentDineSykmeldinger() {
    yield takeEvery(actiontyper.HENT_DINE_SYKMELDINGER_FORESPURT, hentSykmeldingerHvisIkkeHentet);
}
