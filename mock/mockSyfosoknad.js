...
function mockOppdaterSporsmalLokal(server) {
    ...
    server.post('/syfosoknad/api/soknader/:soknadId/sporsmal/:sporsmalId', (req, res) => {
...
function mockOppdaterSporsmalOpplaeringsmiljo(server) {
    server.post('/syfosoknad/api/oppdaterSporsmal', (req, res) => {
        const soknad = req.body;
        res.setHeader('Content-Type', 'application/json');
...
function mockSyfosoknadLokalt(server) {
    server.post('/syfosoknad/api/opprettSoknadUtland', (req, res) => {
    ...
    server.post('/syfosoknad/api/soknader/:id/avbryt', (req, res) => {
    ...
    server.post('/syfosoknad/api/soknader/:id/ettersendTilNav', (req, res) => {
        const soknadId = req.params.id;
        const soknad = mockData[enums.SOKNADER].find((s) => {
            return s.id === soknadId;
        });
    ...
    server.post('/syfosoknad/api/soknader/:id/ettersendTilArbeidsgiver', (req, res) => {
        const soknadId = req.params.id;
        const soknad = mockData[enums.SOKNADER].find((s) => {
            return s.id === soknadId;
        });
    ...
    server.get('/syfosoknad/api/sykmeldinger/:id/actions/erUtenforVentetid', (req, res) => {
        res.json(false);
    });

    server.get('/syfosoknad/api/sykmeldinger/:id/actions/behandlet', (req, res) => {
        res.json(true);
    });

    server.post('/syfosoknad/api/sykmeldinger/:id/actions/skalOppretteSoknad', (req, res) => {
        res.json(true);
    });

    server.post('/syfosoknad/api/soknader/:id/gjenapne', (req, res) => {
        const soknadId = req.params.id;
        mockData[enums.SOKNADER] = mockData[enums.SOKNADER].map((s) => {
            return s.id === soknadId
                ? {
    ...
    server.post('/syfosoknad/api/soknader/:id/send', (req, res) => {
    ...
    server.post('/syfosoknad/api/soknader/:id/korriger', (req, res) => {
        const { id } = req.params;
...
function mockSyfosoknadOpplaringsmiljoOgLokalt(server) {
    server.get('/syfosoknad/api/soknader', (req, res) => {
        res.setHeader('Content-Type', 'application/json');
        res.send(JSON.stringify(mockData[enums.SOKNADER]));
    });

    server.post('/syfosoknad/api/soknader/:id/mottaker', (req, res) => {
        res.setHeader('Content-Type', 'application/json');
        res.send(JSON.stringify({
