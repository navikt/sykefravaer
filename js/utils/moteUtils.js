...
export const erMotePassert = (mote) => {
    ...
    return mote.alternativer.filter(alternativ => alternativ.tid <= newDate()).length === antallAlternativer;
};

export const getMote = state => state.mote && state.mote.data;
...
export const getNyeAlternativer = (mote, deltakertype = BRUKER) => {
    ...
    return mote.alternativer.filter(alternativ => alternativ.created > innloggetBruker.svartidspunkt);
};
...
export const finnDeltakerByType = (deltakere, deltakertype) => deltakere.filter(deltaker => (deltaker.type === deltakertype ? 1 : 0))[0];

export const finnNyesteAlternativ = alternativer => [...alternativer].sort((m1, m2) => m2.created - m1.created)[0];
...
export const getSvarsideModus = (mote, deltakertype = BRUKER) => {
    ...
    if (mote.status === BEKREFTET && mote.alternativer.filter(alternativ => alternativ.created > mote.bekreftetTidspunkt).length === 0) {
        return BEKREFTET;
    }
    ...
    const alleAlternativerErBesvart = mote.alternativer.filter((alternativ) => {
        const svar = deltaker.svar.filter(s => s.id === alternativ.id)[0];
...
export const konverterTid = mote => Object.assign({}, mote, {
    ...
    deltakere: mote.deltakere.map(deltaker => Object.assign({}, deltaker, {
        svartidspunkt: lagJsDate(deltaker.svartidspunkt),
        svar: deltaker.svar.map(s => Object.assign({}, s, {
            tid: lagJsDate(s.tid),
            created: lagJsDate(s.created),
        })),
    })),
    alternativer: mote.alternativer.map(alt => Object.assign({}, alt, {
