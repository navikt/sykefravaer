...
import {
    ANNEN_ARBEIDSGIVER_ORGNUMMER,
    ANNEN_ARBEIDSSITUASJON,
...
export const harValgtArbeidsgiverMedNaermesteLeder = (svar, arbeidsgivere) => {
    ...
    return (
        harValgtArbeidsgiver(svar, arbeidsgivere)
...
export const RendreVelgArbeidssituasjon = ({
    ...
    const hjelpelinje = arbeidsgivere.length > 1 ? (
        <p>
            {getLedetekst(
                'send-til-arbeidsgiver.velg-arbeidsgiver.flere-arbeidsgivere-infotekst',
            )}
        </p>
    ) : null;
    const gdpr = harValgtArbeidsgiver(input.value, arbeidsgivere) ? (
        <p className="sist js-gdpr">
            {getLedetekst('send-til-arbeidsgiver.gdpr.sender-til-altinn')}
        </p>
    ) : null;
...
RendreVelgArbeidssituasjon.propTypes = {
    ...
    alternativer: PropTypes.arrayOf(
        PropTypes.shape({
...
export const visTillegg = (props) => {
    ...
    return (
        !props.harStrengtFortroligAdresse
...
export const getArbeidsgivere = (arbeidsgivere) => {
    return arbeidsgivere.map((a) => {
        return {
            label: a.navn,
            value: a.orgnummer,
            labelSekundaer: `(${getLedetekst(
                'send-til-arbeidsgiver.orgnr',
            )}: ${formaterOrgnr(a.orgnummer)})`,
        };
    });
};
...
export const getArbeidsgivereOgArbeidssituasjoner = (arbeidsgivere) => {
    return [
        ...
        ...arbeidssituasjoner.map((value) => {
            const label = arbeidsgivere.length > 0 && value === ARBEIDSTAKER
                ? getLedetekst(
                    `din-sykmelding.arbeidssituasjon.alternativ.${value.toLowerCase()}-annen-arbeidsgiver.2`,
                )
                : getLedetekst(
                    `din-sykmelding.arbeidssituasjon.alternativ.${value.toLowerCase()}.2`,
                );
...
export const getAlternativer = (arbeidsgivere, state = {}) => {
    ...
    return !state.visArbeidssituasjoner && arbeidsgivere.length > 0
        ? [...getArbeidsgivere(arbeidsgivere), flereValg]
