...
import { tidligsteFom, senesteTom, arbeidssituasjoner, getLedetekst, sykmeldingstatuser } from 'digisyfo-npm';
...
export const mapStateToProps = (state) => {
    ...
    const iDag = new Date();
    const antallBehandledeSykmeldinger = state.sykeforloep.data.length === 0
        ? 0
        : state.sykeforloep.data[0].sykmeldinger.filter((s) => {
    ...
    const antallAktuelleSykmeldinger = state.sykeforloep.data.length > 0
        && state.sykeforloep.data[0].sykmeldinger
            .filter((s) => {
                return s.valgtArbeidssituasjon === arbeidssituasjoner.ARBEIDSLEDIG;
            })
            .filter((s) => {
    ...
    const harAktuelleSykmeldinger = antallAktuelleSykmeldinger > 0 && antallAktuelleSykmeldinger === antallBehandledeSykmeldinger;
